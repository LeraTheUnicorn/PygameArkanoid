...
def test_strict_10_limit() -> bool:
    ...
    try:
        ...
        test_players: List[Tuple[str, int, int]] = [
            ("Player1", 50, 30),
            ("Player2", 49, 35),
            ("Player3", 48, 40),
            ("Player4", 47, 45),
        ...
        for player_name, score, time_seconds in test_players:
            saved: bool = manager.add_score(player_name, score, time_seconds)
            ...
            if saved:
                ...
                rejected_count += 1
                print(
                    f"[NO] {player_name}: {score} очков - ОТКЛОНЕН (не попал в топ-10)"
                )
        ...
        with open(temp_filename, "r", encoding="utf-8") as f:
            saved_scores: List[dict] = json.load(f)
        ...
        for i, score_data in enumerate(saved_scores, 1):
            print(
                f"{i:2d}. {score_data['player_name'][:15]:<15} - {score_data['score']:2d} очков ({score_data['time_formatted']})"
            )
        ...
        success: bool = saved_count == 10 and rejected_count == 5 and len(saved_scores) == 10
        ...
        manager2: HighScoreManager = HighScoreManager()
        manager2.highscores = saved_scores

        # Пытаемся добавить результат, который точно не попадет в топ-10
        not_saved: bool = manager2.add_score("LowScore", 1, 200)

        if not not_saved:
            print("[OK] Система корректно отклоняет низкие результаты")
            print(
                "[OK] Сообщение 'Результат не попал в топ-10, таблица рекордов не обновлена' будет показано пользователю"
            )
        else:
...
def test_existing_highscores() -> None:
    ...
    if os.path.exists("highscores.json"):
        with open("highscores.json", "r", encoding="utf-8") as f:
            existing_scores: List[dict] = json.load(f)
    ...
    top_scores: List[dict] = manager.get_top_scores()
    if len(top_scores) <= 10:
        print(f"[OK] get_top_scores() возвращает {len(top_scores)} записей - корректно")
    else:
        print(
            f"[ERROR] get_top_scores() возвращает {len(top_scores)} записей - должно быть <= 10"
        )
    ...
    if len(manager.highscores) < 10:
        ...
        test_result: bool = manager.is_top_score(10)
    ...
    if len(manager.highscores) == 10:
        ...
        if not test_result:
            print(
                "[OK] Корректно: низкий результат не попадает в топ при полной таблице"
            )
        else:
            print(
                "[ERROR] Ошибка: низкий результат не должен попадать в топ при полной таблице"
            )
