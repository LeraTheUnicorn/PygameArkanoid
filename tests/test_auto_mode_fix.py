...
def test_auto_mode_fix():
    ...
    try:
        ...
        if found_checks >= 2:
            ...
            print(f"   [FAIL] ESC handling logic incomplete (found {found_checks}/3 checks)")
    ...
    try:
        ...
        if total_calls >= 2 and updated_calls == total_calls:
            print(f"   [OK] All show_game_results calls updated ({updated_calls}/{total_calls})")
        else:
            print(f"   [FAIL] Not all show_game_results calls updated ({updated_calls}/{total_calls})")
            return False
    ...
    try:
        ...
        if found_checks == 3:
            ...
            print(f"   [FAIL] Key 0 handling incomplete (found {found_checks}/3 checks)")
            return False
