...
@dataclass
class MockPaddle:
    rect: pygame.Rect = pygame.Rect(400, 580, 120, 15)


@dataclass
class MockBall:
    rect: pygame.Rect = pygame.Rect(400, 400, 16, 16)
    vel_x: int = 3
    vel_y: int = 3
...
def create_test_bricks() -> List[pygame.Rect]:
...
def test_ai_player_creation() -> None:
...
def test_game_state_update() -> None:
    ...
    bricks: List[pygame.Rect] = create_test_bricks()
...
def test_optimal_position_calculation() -> None:
    ...
    optimal_x: int = ai.get_optimal_paddle_position()
...
def test_paddle_movement() -> None:
    ...
    movement: int = ai.move_paddle_towards(paddle.rect.centerx, 9)
