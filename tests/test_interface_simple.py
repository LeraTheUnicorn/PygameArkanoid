...
def test_highscores_logic() -> bool:
    ...
    test_file: str = "test_highscores_simple.json"
    ...
    try:
        ...
        if len(manager.highscores) == 10:
            ...
            print(
                f"[ERROR] Загружено {len(manager.highscores)} рекордов (ожидалось 10)"
            )
            return False
...
def test_strict_10_limit() -> bool:
    ...
    test_file: str = "test_strict_limits.json"
    ...
    try:
        ...
        expected_scores: List[int] = list(range(50, 39, -1))[
            :10
...
def test_interface_changes() -> bool:
    ...
    print(
        "2. [OK] В get_player_name() добавлен вызов show_highscores() с exit_on_esc=False"
    )
    print(
        "3. [OK] В show_game_results() добавлен вызов show_highscores() с exit_on_esc=True"
    )
    ...
    print(
        "- show_highscores(screen, font, highscore_manager, exit_on_esc=False) -> (bool, bool)"
    )
    print(
        "- show_game_results(screen, font, big_font, score, player_name, time, manager) -> (bool, bool)"
    )
...
def main() -> bool:
    ...
    test1: bool = test_highscores_logic()
    test2: bool = test_strict_10_limit()
    test3: bool = test_interface_changes()
