...
class PositionOptimizer:
    ...
    def find_optimal_position(self, game_state: GameState, trajectory_predictor: Any) -> int:
        ...
        try:
            ...
            optimal_bounce_x = trajectory_predictor.find_optimal_bounce_position(
                game_state, paddle_y
            )

            if optimal_bounce_x is not None:
                # Переводим X-координату отскока в позицию центра платформы
                return self._bounce_x_to_paddle_x(
                    optimal_bounce_x, game_state.paddle_width
                )
    ...
    def _follow_ball_position(self, game_state: GameState) -> int:
        ...
        max_x = (
            self.screen_width - game_state.paddle_width // 2 - self.paddle_safety_margin
        )
    ...
    def calculate_paddle_movement(
        self, current_x: int, target_x: int, paddle_speed: int
    ) -> int:
        """
    ...
    def evaluate_position_quality(self, game_state: GameState, paddle_x: int) -> float:
        ...
        center_bonus = 1.0 - abs(paddle_x - self.screen_width // 2) / (
            self.screen_width // 2
        )
    ...
    def find_target_bricks(
        self, game_state: GameState, paddle_x: int
    ) -> List[pygame.Rect]:
        """
    ...
    def _can_reach_brick(
        self, game_state: GameState, paddle_x: int, brick: pygame.Rect
    ) -> bool:
        """
    ...
    def optimize_for_multiple_shots(
        ...
        for test_x in range(50, self.screen_width - 50, 25):
            score = self._evaluate_position_for_sequence(
                game_state, test_x, trajectory_predictor
            )
    ...
    def _evaluate_position_for_sequence(
        ...
        for brick in target_bricks:
            ...
            optimal_pos = self._calculate_optimal_bounce_position(
                game_state, brick, trajectory_predictor
            )
