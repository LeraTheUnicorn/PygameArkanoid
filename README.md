# 🕹️ Игра Арканоид

Классическая игра Арканоид, написанная на Python с использованием Pygame. Включает в себя систему рекордов, звуковые эффекты и современный интерфейс.

![Версия](https://img.shields.io/badge/version-1.4.1-blue.svg)
![Python](https://img.shields.io/badge/python-3.8+-blue.svg)
![Pygame](https://img.shields.io/badge/pygame-2.5+-green.svg)

## 🎮 Особенности

### Геймплей
- **Классический геймплей Арканоид** - отбивайте мяч платформой и разрушайте кубики
- **Цветные секции платформы** - визуальные подсказки направления отскока
- **Шлейф за мячом** - красивый визуальный эффект (20 позиций)
- **Система жизней** - 3 жизни для прохождения уровня
- **Автоматический перезапуск** - по клавише R после окончания игры

### Система рекордов
- **Ввод имени игрока** - персонализированная игра
- **Отсчет времени** - точное время прохождения каждой игры
- **Топ-10 таблица** - сохранение лучших результатов
- **Сортировка результатов** - по очкам, времени и имени
- **Постоянное хранение** - результаты сохраняются в файл

### Звуковые эффекты
- **Процедурная генерация звуков** - 16-битные тональные эффекты
- **Фоновая музыка** - цикличное воспроизведение
- **Звук отскока** - от платформы при ударе мяча
- **Звуки разрушения** - разные частоты для разных ситуаций

### Интерфейс
- **Полная локализация** - русский язык во всем интерфейсе
- **Просмотр рекордов** - по клавише ESC во время игры
- **Подробная статистика** - очки, время, количество отбитий
- **Экран результатов** - детальная информация по завершении

## 🚀 Установка и запуск

### Требования
- Python 3.8 или выше
- Poetry (рекомендуется) или pip

### С Poetry (рекомендуется)
```bash
# Клонирование репозитория
git clone <repository-url>
cd PythonProject2

# Установка зависимостей
poetry install

# Запуск игры
poetry run python PyGameBall.py

# Или активация окружения
poetry shell
python PyGameBall.py
```

### С pip
```bash
# Установка зависимостей
pip install pygame numpy

# Запуск игры
python PyGameBall.py
```

## 🎯 Управление

### В игре
- **← / →** - движение платформы (запуск игры)
- **R** - перезапуск после окончания игры
- **ESC** - просмотр таблицы рекордов
- **H** - просмотр рекордов на экране результатов
- **Enter** - подтверждение выбора

### Ввод имени
- **Клавиши символов** - ввод имени игрока
- **Backspace** - удаление символа
- **Enter** - завершение ввода

## 📊 Структура проекта

```
PythonProject2/
├── PyGameBall.py          # Основной файл игры
├── highscores.py          # Модуль управления рекордами
├── highscores.json        # Файл сохранения результатов
├── pyproject.toml         # Конфигурация Poetry и зависимости
├── changelog.md           # История изменений проекта
├── sounds/                # Папка со звуковыми файлами
│   ├── bounce.wav         # Звук отскока от платформы
│   ├── S31-Night Prowler.ogg  # Фоновая музыка
│   └── Jump*.wav          # (не используются, заменены процедурными)
└── README.md              # Этот файл
```

## 🎵 Звуковая система

### Процедурная генерация звуков
- **Частоты:** 440Гц, 523Гц, 659Гц (ноты A4, C5, E5)
- **Длительность:** 0.2 секунды с экспоненциальным затуханием
- **Формат:** 16-битный стерео, 44.1 кГц
- **Гармоники:** дополнительные частоты для богатого звучания

### Файлы звуков
- `bounce.wav` - оригинальный звук отскока от платформы
- `S31-Night Prowler.ogg` - фоновая музыка в формате OGG

## 🏆 Система рекордов

### Сортировка результатов
1. **По очкам** (по убыванию)
2. **При равных очках** - по времени игры (по возрастанию)
3. **При равных очках и времени** - по имени (алфавитный порядок)

### Сохранение данных
```json
{
  "player_name": "Игрок",
  "score": 42,
  "time_seconds": 85,
  "time_formatted": "1:25",
  "date": "2025-11-28 13:17"
}
```

## 🔧 Технические детали

### Зависимости
- **pygame** (^2.5.0) - игровая графика и звук
- **numpy** (^1.24.0) - генерация звуковых волн

### Ключевые компоненты
- **Ball class** - управление мячом и физикой
- **Paddle class** - управление платформой
- **HighScoreManager** - работа с рекордами
- **generate_tone_sound()** - процедурная генерация звуков

### Оптимизации
- Отрисовка только изменяющихся элементов
- Эффективное управление шлейфом мяча
- Оптимизированная генерация звуков
- Кэширование рекордов в памяти

## 📝 История изменений

Полную историю изменений смотрите в [changelog.md](changelog.md).

### Текущая версия: 1.4.1
- Исправление ошибок pygame.K_H
- Улучшение форматирования таблицы результатов
- Замена звуковых файлов на процедурную генерацию
- Добавление файла pyproject.toml

## 🤝 Участие в разработке

Проект открыт для предложений и улучшений. Основные направления развития:

- Добавление новых уровней
- Улучшение графики и анимаций
- Новые типы бонусов и препятствий
- Мультиплеер режим
- Система достижений

## 📄 Лицензия

Этот проект создан в образовательных целях и может свободно использоваться и модифицироваться.

---

**Приятной игры! 🎮**