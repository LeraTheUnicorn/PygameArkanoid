...
class ZoneHandler:
    ...
    def handle_separation_zone(
        ...
        # ВЫСОКИЙ ПРИОРИТЕТ: Мяч движется вниз и близко к платформе - немедленный расчет
        if ball_vel_y > 0 and separation_zone_start <= ball_y < paddle_zone_start:
            # Мяч движется вниз в зоне разделения - нужно рассчитать позицию
            ...
            if not self.separation_zone_tracker.ball_entered_separation_zone:
        ...
        # СРЕДНИЙ ПРИОРИТЕТ: Мяч движется вверх - занимаем стратегическую позицию
            ...
            # Занимаем стратегическую позицию вместо точного преследования
            if self.separation_zone_tracker.target_position_set:
                self.separation_zone_tracker.target_position_set = False
                self.separation_zone_tracker.target_position = None
            # Возвращаем 0 - не двигаемся, занимаем текущую позицию
            return 0

        # НИЗКИЙ ПРИОРИТЕТ: Мяч далеко или движется горизонтально
        else:
            # Мяч вне зоны разделения или не движется в нужном направлении
