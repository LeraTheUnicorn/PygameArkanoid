...
class LearningSystem:
    ...
    def update_strategy(self, action_result: Dict[str, Any]):
        ...
        if game_state_before and game_state_after:
            self._update_position_preferences(game_state_before, game_state_after, success)
        
        # Анализируем паттерны траекторий
        if "trajectory_prediction" in action_result:
            self._analyze_trajectory_pattern(action_result["trajectory_prediction"], success)
    ...
    def _update_position_preferences(self, state_before: Dict, state_after: Dict, success: bool):
        ...
        position_segment = int(paddle_x_before / 80)  # 80 пикселей на сегмент для экрана 800px
    ...
    def _calculate_trajectory_accuracy(self, predicted: List[Dict], actual: List[Dict]) -> float:
    ...
    def get_strategy_recommendation(self, current_situation: Dict[str, Any]) -> Dict[str, float]:
        """
