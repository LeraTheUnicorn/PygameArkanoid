...
class PerformanceLogger:
    ...
    def __init__(self, session_id: Optional[str] = None):
        ...
        self.session_log_file = os.path.join(self.logs_dir, f"session_{self.session_id}.json")
    ...
    def log_paddle_movement(self, from_x: int, to_x: int, reason: str, confidence: float = 1.0):
    ...
    def log_trajectory_prediction(self, predicted_trajectory: List[Dict], 
    ...
    def log_brick_interaction(self, brick_hit: bool, bricks_destroyed: List[Dict], 
        ...
        interaction_data = {
            ...
            "prediction_accuracy": self._calculate_prediction_accuracy(
                bricks_destroyed, predicted_bricks
            ) if predicted_bricks else None
        }
        
        self.log_action(interaction_data)
    
    def log_decision_making(self, decision_reason: str, considered_options: List[Dict], 
                          chosen_option: Dict, decision_confidence: float):
    ...
    def _calculate_prediction_accuracy(self, actual_bricks: List[Dict], 
                                     predicted_bricks: Optional[List[Dict]]) -> float:
        """Рассчитывает точность предсказания"""
        if not predicted_bricks:
            return 0.0
        
        actual_brick_positions = set((brick.get('x'), brick.get('y')) for brick in actual_bricks)
        predicted_brick_positions = set((brick.get('x'), brick.get('y')) for brick in predicted_bricks)
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        if game_actions:
            ...
            report["game_performance"] = {
                ...
                "success_rate": len(successful_games) / len(game_actions) if game_actions else 0,
                "average_score": sum(g.get("final_score", 0) for g in game_actions) / len(game_actions),
                "average_duration": sum(g.get("game_duration", 0) for g in game_actions) / len(game_actions)
            }
        
        # Анализ движений платформы
        movement_actions = [a for a in self.actions_log if a.get("type") == "paddle_movement"]
        if movement_actions:
            total_distance = sum(a.get("movement_distance", 0) for a in movement_actions)
            report["paddle_movements"] = {
                "total_movements": len(movement_actions),
                "total_distance": total_distance,
                "average_distance": total_distance / len(movement_actions) if movement_actions else 0
            }
        
        # Анализ точности предсказаний
        interaction_actions = [a for a in self.actions_log if a.get("type") == "brick_interaction"]
        if interaction_actions:
            accuracies = [a.get("prediction_accuracy", 0) for a in interaction_actions 
