...
def build_msi():
    ...
    try:
        ...
        exe_path = os.path.join(project_root, 'FINAL_RELEASE', f"Arkanoid_v{version}.exe")
        if not os.path.exists(exe_path):
            print(f"EXE файл {exe_path} не найден. Создаю...")
            exe_result = subprocess.run(['python', os.path.join(project_root, 'scripts', 'build_exe.py')])
            if exe_result.returncode != 0:
                print("Ошибка создания EXE")
                return False

        # Запускаем скрипт создания MSI
        result = subprocess.run(['python', os.path.join(project_root, 'scripts', 'create_msi.py')])

        # Копируем MSI в FINAL_RELEASE и проверяем
        build_msi_path = os.path.join(project_root, 'build', f"Arkanoid_v{version}_Setup.msi")
...
def main():
    ...
    if build_msi():
        version = get_current_version()
        final_msi_path = os.path.join(project_root, 'FINAL_RELEASE', f"Arkanoid_v{version}_Setup.msi")
